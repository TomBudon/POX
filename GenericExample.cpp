...
Norm GenericExample::run(GenericExample::SolverType solverType) {

    if (solverType == GenericExample::SolverType::Hypre) {
        std::cout << "Hypre Solver" << std::endl;
    }
    else if (solverType == GenericExample::SolverType::PETSc) {
        std::cout << "PETSc Solver" << std::endl;
    }
  ...
  auto unique_api = SolverFabric::create(solverType);
...
std::cout << "norm local : " << norm_local << std::endl;

  res.norm = norm;
  res.norm_local = norm_local;
